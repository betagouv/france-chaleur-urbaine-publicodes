...
const modesAvecDimensionnement = [
	...modesDeChauffage,
	{ label: "Réseau de froid", key: "réseau de froid" },
] as const;
...
function tableParMode(
	...
	addTable(
		...
		modes.map((m) => [
			m.label,
			...suffixes.map((s) => evaluate(`${m.key} . ${namespace} . ${s}`)),
		]),
	);
}
...
addTable(
	...
	[
		[
			"Chaleur (RCU)",
			evaluate("combustibles . réseau de chaleur . abonnement"),
			evaluate("combustibles . réseau de chaleur . consommation"),
			"",
		],
		[
			"Froid (RFU)",
			evaluate("combustibles . réseau de froid . abonnement"),
			evaluate("combustibles . réseau de froid . consommation"),
			"",
		],
		[
			"Electricité indiv",
		...
		[
			"Electricité coll",
			evaluate("combustibles . électricité . abonnement collectif"),
			evaluate("combustibles . électricité . consommation collectif"),
			"",
		],
		[
			"Gaz individuel",
			evaluate("combustibles . gaz . abonnement individuel"),
			evaluate("combustibles . gaz . consommation individuel"),
			"",
		],
		[
			"Gaz collectif",
			evaluate("combustibles . gaz . abonnement collectif"),
			evaluate("combustibles . gaz . consommation collectif"),
			"",
		],
...
tableParMode(
	...
	[
		...
		"Investissement par lgt type / tertiaire (€)",
...
tableParMode(
	...
	[
		"Installation",
		"Coût combustible abonnement (P1 abo) €TTC/an",
		"Coût combustible consommation (P1 conso) €TTC/an",
		"Coût électricité auxiliaire (P1') €TTC/an",
		"Coût combustible ballon ECS à accumulation (P1 ECS) €TTC/an",
		"Coût combustible ballon ECS solaire (P1 ECS) €TTC/an",
	],
	"coûts",
	[
		"P1 abonnement",
...
tableParMode(
	...
	[
		...
		"Puissance nécessaire chauffage (kW)",
		"Puissance nécessaire ECS (kW)",
		"Puissance nécessaire refroidissement (kW)",
...
tableParMode(
	...
	[
		"Installation",
		"Besoin d'installation supplémentaire pour ECS ?",
		"Volume du ballon ECS (L)",
		"Consommation d'électricité (kWh/an)",
		"Appoint d'électricité (kWh/an)",
	],
	"installation",
	[
...
tableParMode(
	...
	[
		...
		"Consommation d'électricité chauffage/refroidissement et ECS (kWh/an)",
	],
	"installation",
...
addTable(
	...
	[
		"Installation",
		"Besoin installation supplémentaire ECS ?",
		"Scope 1 - Besoins chauffage et ECS si même équipement (kgCO2 équ.)",
		"Scope 2 - Auxiliaires et combustible électrique (kgCO2 équ.)",
		"Scope 2 - ECS solaire thermique",
		"Scope 2 - ECS avec ballon électrique",
		"Scope 2 - Total",
		"Scope 3",
		"Total des émissions",
	],
	modesDeChauffage.map((m) => [
		...
		evaluate(
			`${m.key} . environnement . besoins de chauffage et ECS si même équipement`,
		),
		evaluate(
			`${m.key} . environnement . auxiliaires et combustible électrique`,
		),
